import subprocess, datetime 
#creates a nodes file (.nod.xml)
def nodgenerator(l,n): 
	with open("input/nodes.nod.xml", 'w') as nodes:
		print >> nodes, ' <!-- Generated by networkgenerator.py on {} -->'.format(datetime.datetime.now())
		print >> nodes, '<nodes>'
		for i in range(n*n): # i = 0,1,2...n
			x = (i%n)*l			
			y = (n-1-i//n)*l	
			print >> nodes, '	<node id="{}" x="{}" y="{}" type="traffic_light" />'.format(str(i), str(x), str(y))
		#auxiliar nodes
		for i in range(n):
			#top nodes
			print >> nodes, '	<node id="{}" x="{}" y="{}" />'.format(str(n*n+i), str(i*l), str(n*l))
			#bottom nodes
			print >> nodes, '	<node id="{}" x="{}" y="{}" />'.format(str(n*n+n+i), str(i*l), str(-l))
			#right nodes
			print >> nodes, '	<node id="{}" x="{}" y="{}" />'.format(str(n*n+2*n+i), str(n*l), str(i*l))
			#left nodes
			print >> nodes, '	<node id="{}" x="{}" y="{}" />'.format(str(n*n+3*n+i), str(-l), str(i*l))
		print >> nodes, '</nodes>'

def edgegenerator(n):
	with open("input/edges.edg.xml", 'w') as edges:
		print >> edges, ' <!-- Generated by networkgenerator.py on {} -->'.format(str(datetime.datetime.now()))
		print >> edges, '<edges>'
		#the edges are build following rows and columns
		for i in range(n):
		#i indicates the column, j the edge in the column
			for j in range(n-1):
				origin = i%5+5*j
				destination = i%5+5*(j+1)
				# one street in each direcion
				if (i%2 == 1):
					origin, destination = destination, origin			
				print >> edges, '	<edge from="{orig}" id="{orig}to{dest}" to="{dest}" numLanes="3"/>'.format(orig=str(origin), dest=str(destination))
		#columns
		for i in range(n):
		#i indicates the row, j the edge in the row
			for j in range(n-1):
				origin = 5*i+j
				destination = 5*i+j+1
				if (i%2 == 1):
					origin, destination = destination, origin
				print >> edges, '	<edge from="{orig}" id="{orig}to{dest}" to="{dest}" numLanes="3" />'.format(orig=str(origin), dest=str(destination))
		# auxiliary edges
		for i in range(n): 
			# top			
			origin = n*n+i;
			destination = i;
			if (i%2 == 1):
				origin, destination = destination, origin 
			print >> edges, '	<edge from="{orig}" id="{orig}to{dest}" to="{dest}" numLanes="3"/>'.format(orig=str(origin), dest=str(destination))
			# bottom
			origin = n*n+n+i
			destination = n*(n-1)+i
			if (i%2 == 0):
				origin, destination = destination, origin
			print >> edges, '	<edge from="{orig}" id="{orig}to{dest}" to="{dest}" numLanes="3"/>'.format(orig=str(origin), dest=str(destination))
			# right
			origin = n*i+n-1
			destination = n*n+2*n+n-i-1
			if (i%2 == 1):
				origin, destination = destination, origin
			print >> edges, '	<edge from="{orig}" id="{orig}to{dest}" to="{dest}" numLanes="3"/>'.format(orig=str(origin), dest=str(destination))
			# left
			origin = n*n+3*n+i
			destination = n*(n-i-1)
			if (i%2 == 1):
				origin, destination = destination, origin
			print >> edges, '	<edge from="{orig}" id="{orig}to{dest}" to="{dest}" numLanes="3"/>'.format(orig=str(origin), dest=str(destination))
		print >> edges, '</edges>'

def networkgenerator(l,n):
	nodgenerator(l,n) 
	edgegenerator(n)
	netconvertProcess = subprocess.call(["netconvert", "-n", "input/nodes.nod.xml", "-e", "input/edges.edg.xml", "-o", "input/mapa.net.xml"])

