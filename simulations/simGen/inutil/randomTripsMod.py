...
def get_options(args=None):
    ...
    optParser.add_option("--size", type="int", default=5, help="size of the grid network generated")
    optParser.add_option("-m", "--maxflow", type="float", default=0, help="maxflow parameter used in the generation of a main flow (between 0 and 180")
    optParser.add_option("--mfbegin", type="int", default=45, help="time (min) of the beginning of the main flow")
    optParser.add_option("--mfend", type="int", default=100, help = "time (min) of the end of the main flow increase, it reaches it's peak then and remains constant")
...
def main(options):
    ...
    def generate_one(idx):
        ...
        return idx + 1
	#function for the edges of the main flow
    def edgeId(a, n):
            if (a/n == 0): # top nodes
                origin = n*n+a
                destination = a%n
                ...
                return '{}to{}'.format(str(origin),str(destination))
            if (a/n == 1): # bottom nodes
                origin = n*n+a
                destination = n*(n-1)+a%n
                if ((a%n)%2 == 0):
                    origin, destination = destination, origin
                return '{}to{}'.format(str(origin),str(destination))
            if (a/n == 2): #right nodes
                origin = n*(a%n)+n-1
                destination = n*n+2*n+n-a%n-1
                if ((a%n)%2 == 1):
                    origin, destination = destination, origin
                return '{}to{}'.format(str(origin),str(destination))
            if (a/n == 3): #left nodes
                origin = n*n+3*n+a%n
                destination = n*(n-a%n-1)
                if ((a%n)%2 == 1):
                    origin, destination = destination, origin
                return '{}to{}'.format(str(origin),str(destination))
    
    def generate_main(idx):
		fouttrips.write( '	<trip id="{}" depart="{}" {} />\n'.format(str(idx), str(i), maintrip))
		return idx + 1
    #end
    
    with open(options.tripfile, 'w') as fouttrips:
        fouttrips.write("""<?xml version="1.0"?>
<!-- generated on %s by $Id: randomTripsMod.py 19495 2015-12-02 12:36:28Z behrisch $
  options: %s
-->
<trips>
                ...
                if options.binomial is None:
                    ...
                    for i in range(options.binomial):
                        if random.random() < prob:
                            idx = generate_one(idx)
                    depart += 1
            #modification to add the main flow
            source = 0;
            destination = 9
            maintrip =  'from="{}" to="{}"'.format(edgeId(source, options.size),edgeId(destination, options.size))
            for i in range(int(options.end)):
                if (random.uniform(0,1) < flowDist.linear(i,options.mfbegin*60, options.mfend*60, options.maxflow)):
                    idx = generate_main(idx)
